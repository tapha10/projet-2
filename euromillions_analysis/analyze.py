"""
Analyse statistique EuroMillions (2020-2026) et génération d'une combinaison
expérimentale unique.

AVERTISSEMENT : ceci est une exploration statistique à but ludique/éducatif.
Chaque tirage EuroMillions est indépendant et équiprobable. Aucune méthode
statistique ne peut augmenter la probabilité réelle de gagner. Voir le
rapport final pour le rappel honnête des probabilités.
"""
...
RAW_PATH = "euromillions_analysis/data/euromillions_202002.csv"
REPORT_PATH = "euromillions_analysis/rapport.md"
RANDOM_SEED = None  # None => aléa contrôlé (basé sur l'horloge) à chaque exécution

N_MAIN, N_STAR = 5, 2
MAIN_MIN, MAIN_MAX = 1, 50
STAR_MIN, STAR_MAX = 1, 12
TOTAL_COMBINATIONS = 139_838_160

N_CANDIDATES = 120_000
N_MONTECARLO_RANDOM = 10_000
N_BACKTEST_DRAWS = 150  # nb de tirages récents utilisés pour le backtesting
...
def detect_columns(df: pd.DataFrame) -> dict:
    ...
    main_cols = []
    for c in cols:
        if re.fullmatch(r"boule[_\s]?\d+", lower[c].strip()):
            main_cols.append(c)
    if not main_cols:
        main_cols = [c for c in cols if re.search(r"\bboule\b", lower[c]) and "gagnante" not in lower[c]]
    main_cols = sorted(main_cols, key=lambda c: int(re.search(r"\d+", c).group()))[:N_MAIN]

    star_cols = []
    for c in cols:
        if re.fullmatch(r"etoile[_\s]?\d+", lower[c].strip()) or re.fullmatch(r"étoile[_\s]?\d+", lower[c].strip()):
            star_cols.append(c)
    if not star_cols:
        star_cols = [c for c in cols if re.search(r"toile", lower[c]) and "gagnante" not in lower[c]]
    star_cols = sorted(star_cols, key=lambda c: int(re.search(r"\d+", c).group()))[:N_STAR]

    if date_col is None or len(main_cols) != N_MAIN or len(star_cols) != N_STAR:
        raise ValueError(
            f"Détection de colonnes incomplète: date={date_col}, "
            f"numéros={main_cols}, étoiles={star_cols}"
        )
    return {"date": date_col, "main": main_cols, "star": star_cols}
...
def load_and_clean(path: str) -> tuple[pd.DataFrame, dict]:
    ...
    valid_star = df[star_cols].apply(
        lambda row: row.between(STAR_MIN, STAR_MAX).all() and row.nunique() == N_STAR, axis=1
    )
    ...
    df[star_cols] = np.sort(df[star_cols].values.astype(int), axis=1)
...
def build_number_stats(df: pd.DataFrame, main_cols: list, star_cols: list) -> dict:
    ...
    last100 = main_binary[-100:] if n_draws >= 1 else main_binary
    last50 = main_binary[-50:] if n_draws >= 1 else main_binary
    starlast100 = star_binary[-100:]
    starlast50 = star_binary[-50:]

    # trend: freq last 50 vs freq previous 50 (50..100 back)
...
def combo_descriptive(df: pd.DataFrame, main_cols: list, star_cols: list) -> dict:
    ...
    star_pair_counts = {}
    for row in star_vals:
        a, b = sorted(row)
        star_pair_counts[(int(a), int(b))] = star_pair_counts.get((int(a), int(b)), 0) + 1
    top_star_pairs = sorted(star_pair_counts.items(), key=lambda kv: -kv[1])[:10]

    sums = main_vals.sum(axis=1)
    evens = (main_vals % 2 == 0).sum(axis=1)
    odds = N_MAIN - evens
    low = (main_vals <= 25).sum(axis=1)
    high = N_MAIN - low

    decade_counts = np.zeros(5, dtype=int)  # 1-10,11-20,21-30,31-40,41-50
    for row in main_vals:
        for v in row:
            decade_counts[min(int(v) - 1, 49) // 10] += 1
    ...
    return {
        ...
        "star_pair_counts": star_pair_counts,
    }
...
def correlation_and_chi2(main_binary: np.ndarray, star_binary: np.ndarray, n_draws: int) -> dict:
    ...
    for a in range(MAIN_MAX - MAIN_MIN + 1):
        for b in range(a + 1, MAIN_MAX - MAIN_MIN + 1):
            ...
            if pa > 0 and pb > 0 and pab > 0:
                lift = pab / (pa * pb)
                lifts[(a + MAIN_MIN, b + MAIN_MIN)] = lift
                cond_freq[(a + MAIN_MIN, b + MAIN_MIN)] = pab / pa if pa > 0 else np.nan
    ...
    return {
        ...
        "n_significant_lift_pairs_bonferroni": None,  # computed below via binomial test
    }
...
def significant_pairs_bonferroni(main_binary: np.ndarray, n_draws: int, n_pairs_tested: int) -> int:
    ...
    p_single = N_MAIN / (MAIN_MAX - MAIN_MIN + 1)
    p_pair_expected = p_single * (N_MAIN - 1) / (MAIN_MAX - MAIN_MIN)  # approx hypergeometric co-occurrence prob
    sig_count = 0
...
def score_combo_for_backtest(main_freq_rate: np.ndarray, star_freq_rate: np.ndarray,
                              cand_main: np.ndarray, cand_star: np.ndarray) -> float:
    return float(main_freq_rate[cand_main - MAIN_MIN].sum() + star_freq_rate[cand_star - STAR_MIN].sum())


def model_predict(hist_main_binary: np.ndarray, hist_star_binary: np.ndarray, rng: np.random.Generator) -> tuple:
    """Simple, leakage-free model: pick weighted-random numbers favoring
    numbers with higher recent (last 100 draws) frequency, tempered by
    overall frequency, mimicking the scoring approach used for the final
    selection but lightweight enough for 10k+ backtest iterations."""
    ...
    main_rate_total = hist_main_binary.mean(axis=0)
    main_rate_recent = recent_main.mean(axis=0)
    main_weight = 0.5 * main_rate_total + 0.5 * main_rate_recent + 1e-6

    star_rate_total = hist_star_binary.mean(axis=0)
    star_rate_recent = recent_star.mean(axis=0)
    star_weight = 0.5 * star_rate_total + 0.5 * star_rate_recent + 1e-6
...
def backtest(df: pd.DataFrame, main_cols: list, star_cols: list, main_binary: np.ndarray, star_binary: np.ndarray) -> dict:
    ...
    if n_test <= 0:
        raise ValueError("Pas assez de tirages pour un backtesting fiable.")
    ...
    n_random_per_draw = N_MONTECARLO_RANDOM // n_test
    n_random_per_draw = max(n_random_per_draw, 20)
    ...
    dist = {k: float(np.mean(model_main_hits == k)) for k in range(0, 6)}
...
def score_candidate(main_nums: np.ndarray, star_nums: np.ndarray, ctx: dict) -> float:
    ...
    star_pair_counts = ctx["star_pair_counts"]
    last_draw_main = ctx["last_draw_main"]
    last_draw_star = ctx["last_draw_star"]
    sum_mean = ctx["sum_mean"]
    sum_std = ctx["sum_std"]
    max_pair_count = ctx["max_pair_count"]
    max_star_pair_count = ctx["max_star_pair_count"]

    score = 0.0

    # --- (a) équilibre fréquence historique / récente (poids fort) ---
    hist_component = 0.0
    ...
    # --- (b) associations de paires (poids FAIBLE, explicite) ---
    pair_component = 0.0
    ...
    for a, b in combinations(sorted(star_nums.tolist()), 2):
        c = star_pair_counts.get((a, b), 0)
        pair_component += c / max_star_pair_count if max_star_pair_count else 0
    score += 3.0 * pair_component  # poids volontairement faible

    # --- (c) somme proche de la zone centrale historique ---
    s = int(main_nums.sum())
    z = abs(s - sum_mean) / sum_std if sum_std else 0
    score += 8.0 * max(0.0, 1.0 - z / 2.5)

    # --- (d) équilibre pair/impair ---
    evens = int(np.sum(main_nums % 2 == 0))
    score += 5.0 if evens in (2, 3) else (2.0 if evens in (1, 4) else 0.0)

    # --- (e) équilibre bas/haut ---
    low = int(np.sum(main_nums <= 25))
    score += 5.0 if low in (2, 3) else (2.0 if low in (1, 4) else 0.0)

    # --- (f) répartition dizaines ---
    decades = set(min(int(n) - 1, 49) // 10 for n in main_nums)
    score += 4.0 * (len(decades) / N_MAIN)

    sorted_main = np.sort(main_nums)

    # --- (g) pénalité suites trop longues ---
    ...
    # --- (h) pénalité similarité avec derniers tirages ---
    overlap = len(set(sorted_main.tolist()) & set(last_draw_main.tolist()))
    if overlap >= 3:
        score -= 4.0 * (overlap - 2)
    star_overlap = len(set(star_nums.tolist()) & set(last_draw_star.tolist()))
    if star_overlap == 2:
        score -= 2.0

    # --- (i) pénalité "profils populaires" (partage du jackpot, pas de proba) ---
...
def generate_candidates(n: int, rng: np.random.Generator) -> tuple:
    ...
    for i in range(n):
        ...
        stars[i] = np.sort(rng.choice(np.arange(STAR_MIN, STAR_MAX + 1), size=N_STAR, replace=False))
    return mains, stars
...
def build_final_selection(df, main_cols, star_cols, number_stats, star_stats, combo_desc, seed_rng) -> dict:
    ...
    ctx = {
        ...
        "star_pair_counts": combo_desc["star_pair_counts"],
        ...
        "max_star_pair_count": max(combo_desc["star_pair_counts"].values()) if combo_desc["star_pair_counts"] else 1,
    }
    ...
    top_k = order[:50]  # petit pool pour l'aléa contrôlé final
    ...
    # Percentile rank within the full candidate pool (not min-max scaling,
    # which would trivially sit near 100 since the pick always comes from
    # the top of the pool).
    ...
    return {
        ...
        "last_draw_main": last_draw_main,
        "last_draw_star": last_draw_star,
        "score_pool_min": float(score_min),
        "score_pool_max": float(score_max),
    }
...
def main():
    ...
    print(f"Numéros: {'-'.join(f'{n:02d}' for n in final['main'])} | Étoiles: {'-'.join(f'{e:02d}' for e in final['star'])}")
...
def write_report(clean_report, stats_data, combo_desc, corr_chi2, bt, final, rng_seed):
    ...
    odds = N_MAIN - evens
    low = int(np.sum(final["main"] <= 25))
    high = N_MAIN - low
    decade_labels = ["1-10", "11-20", "21-30", "31-40", "41-50"]
    decade_repr = {}
    for n in final["main"]:
        d = decade_labels[min(int(n) - 1, 49) // 10]
    ...
    if bt["pvalue"] < 0.05 and bt["mean_diff"] > 0:
        bt_conclusion = (
            ...
            "prudence : il peut s'agir de surapprentissage (le modèle a été ajusté sur les "
            "mêmes données historiques utilisées pour le tester) ou d'un résultat dû au "
            "hasard des multiples comparaisons. Aucune conclusion de supériorité réelle "
            "n'est retenue sans validation sur des tirages futurs indépendants."
        )
    ...
    lines = []
    lines.append("# Rapport d'analyse statistique EuroMillions (2020-2026)\n")
    lines.append(
        "**Avertissement** : cette analyse est une exploration statistique expérimentale et "
        "ludique. EuroMillions est un tirage aléatoire indépendant ; aucune méthode statistique "
        "ne peut prédire ou influencer un tirage futur. La grille produite ne doit pas être "
    ...
    lines.append(
        f"- Lignes supprimées (valeurs hors plage 1-50/1-12 ou doublons de numéros dans la même ligne) : "
    ...
    lines.append(
        f"- Période : {clean_report['first_date'].date()} → {clean_report['last_date'].date()}\n"
    )
    ...
    lines.append(", ".join(f"{a}-{b} ({c}x)" for (a, b), c in combo_desc["top_star_pairs"][:5]))
    lines.append(
    ...
    lines.append(f"- Bas (1-25) / haut (26-50) moyens : {combo_desc['low_mean']:.2f} / {combo_desc['high_mean']:.2f}")
    lines.append(
    ...
    lines.append(
        f"- Test du khi-deux (étoiles) : χ²={corr_chi2['chi2_star']:.2f}, p-value={corr_chi2['p_star']:.4f} "
    ...
    lines.append(f"- Moyenne de bonnes étoiles du modèle : {bt['model_mean_star_hits']:.4f}")
    lines.append(f"- Moyenne de bonnes étoiles du hasard : {bt['random_mean_star_hits']:.4f}")
    lines.append(
    ...
    lines.append(f"- Combinaisons candidates générées et scorées : {N_CANDIDATES:,}".replace(",", " "))
    lines.append(
    ...
    lines.append(f"**Étoiles : {' – '.join(f'{e:02d}' for e in final['star'])}**\n")
    ...
    lines.append(f"- Nombre total de combinaisons possibles : {total_fmt}")
    lines.append(f"- Avant analyse : 1 sur {total_fmt}, soit environ {prob_before*100:.9f}%")
    lines.append(f"- Après analyse : 1 sur {total_fmt}, soit environ {prob_after*100:.9f}%")
    ...
    lines.append(
        ...
        "La probabilité réelle de gain au jackpot reste strictement 1 sur 139 838 160, identique "
        "avant et après toute analyse.\n"
    )
